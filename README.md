...
- **面剔除 + 纹理图集 + 贪心网格合并**:每区块按面方向做切片贪心合并(同面同材质的连续面合并为大四边形),三角形数比逐块面降 ~70%;合并后贴图经 shader `fract` 平铺逐块重复,每区块合并为至多 3 个 BufferGeometry(实体/树叶/水),1–3 draw call
...
- **准星**:黑白描边,亮暗背景都清晰可见
- **UI**:toast 上移不挡准星;破坏进度条 + 方块表面 10 阶裂纹贴图(不再刷屏)
