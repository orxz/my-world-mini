...
- **同面同材质连续面合并**:`buildChunkMesh` 重写为逐面方向切片扫描 + 贪心扩宽扩高,
  ...
  272k → 89k(**降 67%**),广场区块 1320 → 344(**降 74%**)
- **贴图平铺 shader 补丁**(r160):几何 uv 改存块级局部坐标(0..w/h),新增 `aTile`
  顶点属性存贴图原点,`onBeforeCompile` 在 `uv_vertex` 后覆写 `vMapUv =
  aTile + fract(uv) * uTileSize` —— 大四边形内每个方块仍完整显示原贴图;
  水(无贴图)不参与;角序/绕向/顶点色与旧逐块面同构(w=h=1 时等价)
- **验证**:拓扑(贪心 quad 反推的可见面覆盖集合 == 独立扫描,652==652 零差异)+
  渲染(固定昼夜后与逐块参考实现逐像素对比,差异 163/4.6M = 0.0035%,抗锯齿边缘级)
- **冒烟测试**:新增 T8(aTile 属性存在 + 三角形数严格低于逐块参考),共 27 项
